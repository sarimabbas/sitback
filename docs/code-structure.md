...
- Dashboard feature is organized under `packages/web/src/features/dashboard/` and is mounted on both `packages/web/src/routes/index.tsx` (default entry) and `packages/web/src/routes/dashboard.tsx`; metadata/todo/tag server functions in `packages/web/src/server/dashboard.ts` (`fetchDashboardMetadata`, `fetchDashboardTodos`, `createDashboardTodo`, `updateDashboardTodo`, `deleteDashboardTodos`, `setDashboardTodoPredecessors`, `renameDashboardTag`, `deleteDashboardTag`); live TanStack DB query collection in `packages/web/src/db-collections/dashboard-todos.ts` (750ms polling + optimistic insert/update/delete persistence); modal-driven editing in `packages/web/src/features/dashboard/components/todo-modal.tsx`; and shared transforms in `packages/web/src/features/dashboard/lib/*` including force-layout graph mapping.
